...
newaction {
    trigger = "release",
    description = "Zoomin: Creates a release of Zoomin",
    execute = function ()
        ...
        local have_signtool = have_required_tool("signtool", { "c:\\Program Files (x86)\\Windows Kits\\10\\bin\\10.0.22000.0\\x86" })
        ...
        if not os.isdir(src .. ".") or not x86_ok then
...
newoption {
    trigger     = "nosign",
    description = "Zoomin: don't sign the release files"
}

--------------------------------------------------------------------------------
newaction {
    trigger = "manifest",
    description = "Zoomin: generate app manifest",
    execute = function ()
        toolchain = _OPTIONS["vsver"] or "vs2022"
